...
#        return neighbours
    ...
    def bfs3(self):
        ...
        distances = empty_arr((len(self.lines), len(self.lines[0]), 11, 4), MAX_DISTANCE)
        distances[0][0][10] = [self.lines[0][0] for _ in range(4)]
        parents = empty_arr((len(self.lines), len(self.lines[0]), 11, 4), None)
        queue = deque()
        for d in range(4):
            queue.append((0, 0, 10, d))
        step = 0
        while len(queue) > 0:
            if step % 100000 == 0:
                print(len(queue))
            ...
            if d == 0:
                ...
                if z <= 6:
                    possible_next.append((x, y+1, 9, 1))
                    possible_next.append((x, y-1, 9, 3))
            if d == 1:
                possible_next.append((x, y+1, z-1, d))
                if z <= 6:
                    possible_next.append((x+1, y, 9, 0))
                    possible_next.append((x-1, y, 9, 2))
            if d == 2:
                possible_next.append((x-1, y, z-1, d))
                if z <= 6:
                    possible_next.append((x, y+1, 9, 1))
